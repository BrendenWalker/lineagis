...
- **CLI workflows**
  - `AC-OV-001`, `AC-OV-002`
  - Coverage:
    - `cmd/lineagis/main_test.go`
    - `cmd/lineagis/inspect_test.go`
...
- **Operator machine validation:** download `lineagis-binaries-windows-amd64` (Windows) or `lineagis-binaries-linux-amd64` (Linux/WSL) from the CI `build` job — do not `go build` from source. See [operator-validation.md](../guides/operator-validation.md).
