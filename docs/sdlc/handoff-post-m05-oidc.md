...
```
Continue Verity post-M05 OIDC. main includes PR for story/8-oidc-auth (GitHub OIDC + dev bearer).

Done: JWT verify (go-oidc), VERITY_OIDC_ISSUER/AUDIENCE, dev token fallback, gh/* namespace repository/ref checks on RegisterDigest/SetTag/putArtifact.
...
```
Continue Verity. main @ <sha> includes M05 OIDC and M06 publish.
...
```
Continue Verity. main includes verity publish + API RegisterDigest/SetTag; OIDC for API auth.

Open: #44 / E08 — integrate Sigstore/cosign so publish signs manifest digest by default (FR-SIGN-001–004).

Read docs/specs/02-signing-verification.md, cmd/verity/publish.go, internal/publish/.
...
## 4. verity inspect + E2E — issue #55

```
Continue Verity. main has publish; signing/policy may be partial.

Open: #55 — AC-OV-001/002 automated e2e: publish → inspect signed artifact.
Blocked until: verity inspect command (FR-DX-002, FR-SIGN-005) exists.
