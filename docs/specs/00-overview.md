...
Verity is an open-source trust platform for publishing, verifying, and governing software artifacts. The MVP delivers a minimal but compelling workflow for open-source maintainers: publish artifacts to OCI storage with cryptographic signatures, attach provenance and SBOMs, enforce basic policies, and verify trust before consumption—all via a CLI-first experience with GitHub Actions integration.
...
- **Phase 1 (implementation milestone):** All Must items.
- **Phase 2 (implementation milestone):** Should items, especially provenance, GitHub Actions, expanded policies, and verification tooling depth.
...
**Example inspect output** (target; Should items may show `⚠` or omit line until implemented):
...
| ID | Priority | Story |
|----|----------|-------|
| US-OV-001 | Must | As a maintainer, I want to publish signed artifacts with one command so that consumers can trust my releases. |
| US-OV-002 | Must | As a consumer, I want to inspect an artifact and see a clear trust summary so that I can decide whether to use it. |
| US-OV-003 | Should | As a maintainer, I want GitHub Actions to publish and sign automatically so that I do not manage long-lived keys. |
| US-OV-004 | Should | As an operator, I want to require signatures for a namespace so that unsigned artifacts cannot be published. |
...
|----|----------|---------------|
| OQ-OV-001 | Should policy evaluation default to push-time only, verify-time only, or both? | [04-policy-enforcement.md](04-policy-enforcement.md) (OQ-POL-001), [api.md](api.md) |
